...
func (m *AdoptArchiveRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SrcKey) > 0 {
		i -= len(m.SrcKey)
		copy(dAtA[i:], m.SrcKey)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.SrcKey)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.SpaceId) > 0 {
...
func (m *AdoptArchiveRequest) SizeVT() (n int) {
	...
	if m.CompressedSize != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.CompressedSize))
	}
	if m.UncompressedSize != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.UncompressedSize))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *AdoptArchiveRequest) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: AdoptArchiveRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: AdoptArchiveRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SrcKey", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.SrcKey = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
			...
			m.OldHash = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
		...
		case 5:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field Eager", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
		...
		case 6:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.CompressedSize |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
...
func (m *AdoptArchiveResponse) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: AdoptArchiveResponse: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: AdoptArchiveResponse: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.Result |= AdoptArchiveResult(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
